/**
 * Minimum seed dataset per the assignment's deliverables:
 *   - 2 orgs
 *   - 1 accepted cross-org connection
 *   - a handful of tickets/PRs
 *   - at least one cross-org share, to demonstrate isolation + sharing
 *     out of the box
 *   - a user for every role, so the interviewer can log in as each one
 *     and see different views without you narrating it
 *
 * Run with: npx ts-node scripts/seed.ts  (after `prisma migrate dev`)
 */
...
const DEMO_PASSWORD = "Passw0rd!"; // documented in README test-credentials table
...
async function main() {
  ...
  // Deliberately NOT shared — used by the BOLA test to assert Globex users
  // cannot reach it even via a manipulated ID.
  await prisma.ticket.create({
    data: {
      orgId: globex.id,
  ...
  // The one explicitly shared item — Globex's guest can see this ticket only.
  ...
  console.log(`Test credentials (password for all: ${DEMO_PASSWORD}):`);
  console.log("  admin@acme.test     -> ORG_ADMIN (Acme)");
  console.log("  agent@acme.test     -> SUPPORT_AGENT (Acme)");
  console.log("  reviewer@acme.test  -> REVIEWER_APPROVER (Acme)");
  console.log("  guest@globex.test   -> CROSS_ORG_GUEST (Globex, shared into Acme's ticket1)");
  console.log("  super@platform.test -> PLATFORM_SUPER_ADMIN (Acme)");
}
